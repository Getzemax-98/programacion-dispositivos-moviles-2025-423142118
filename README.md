# Programación de Dispositivos Móviles - 2025

Repositorio para las actividades de la asignatura **Programación de Dispositivos Móviles**, impartida por Juan Manuel Martínez Fernández.

## 📚 Información del Curso
- **Universidad**: UNAM
- **Asesor**: Juan Manuel Martínez Fernández
- **Periodo**: Agosto - Diciembre 2025
- **Tecnologías**: React Native, Expo, JavaScript, Node.js, SQLite

## 🗂️ Estructura del Repositorio
programacion-dispositivos-moviles-2025/
- ├── unidad-1/ # Introducción a tecnologías móviles
- ├── unidad-2/ # Características del desarrollo móvil
- ├── unidad-3/ # Fundamentos de programación
- ├── unidad-4/ # Programación para iOS
- ├── unidad-5/ # Programación para Android
- └── unidad-7/ # Desarrollo de aplicación móvil

## 🔗 Enlaces Útiles
- [Plataforma Expo](https://expo.dev)
- [Snack Expo](https://snack.expo.dev)
- [Documentación React Native](https://reactnative.dev)

---
*Última actualización: $(date)*
